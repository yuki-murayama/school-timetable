 ...
 */
export async function clickWithOverlayBypass(
  page: Page,
  selector: string,
  description?: string
): Promise<boolean> {
  ...
  const clickMethods = [
    ...
    { name: 'Force click', method: () => element.click({ force: true }) },
    {
      ...
      method: () => element.evaluate(el => (el as HTMLElement).click()),
    },
    {
    ...
    },
    {
      ...
      method: async () => {
        ...
        await page.evaluate(() => {
          ...
          bgOverlays.forEach((overlay: Element) => {
            const overlayElement = overlay as HTMLElement
            if (overlayElement?.classList.contains('fixed')) {
              overlayElement.style.pointerEvents = 'none'
              overlayElement.style.visibility = 'hidden'
              overlayElement.style.display = 'none'
            }
    ...
    },
    {
      ...
      method: () =>
        element.evaluate((el: HTMLElement) => {
          ...
          if (reactKey) {
            const elementWithFiber = el as HTMLElement & { [key: string]: unknown }
            const fiberNode = elementWithFiber[reactKey]
            if (fiberNode?.memoizedProps?.onClick) {
...
export async function waitAndClick(
  ...
  try {
    ...
    return await clickWithOverlayBypass(page, selector, description)
  } catch (error) {
...
export async function selectDropdownOption(
  ...
  try {
    // ドロップダウンのトリガーをクリック
    const triggerClicked = await waitAndClick(page, triggerSelector, {
      timeout,
    ...
    for (const optionSelector of optionSelectors) {
      ...
      if ((await optionElement.count()) > 0) {
        const optionClicked = await clickWithOverlayBypass(
          page,
          optionSelector,
 ...
 * 開いているモーダルを閉じる
 */
export async function closeAnyOpenModal(page: Page): Promise<void> {
  try {
    ...
    if (
      (await page.locator(overlaySelector).count()) > 0 ||
