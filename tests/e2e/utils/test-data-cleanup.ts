...
export class TestDataCleanup {
   ...
   * 行からIDを抽出
   */
  private async extractIdFromRow(row: unknown, type: string): Promise<string | null> {
    try {
      // 編集ボタンからIDを抽出
  ...
  private async navigateToDataSection(type: 'teacher' | 'subject' | 'classroom') {
    ...
    await this.page.click(
      `[role="tab"][data-state*="${tabValues[type]}"], button[data-value="${tabValues[type]}"], [role="tab"]:has-text("${this.getTabLabel(type)}")`
    )
  ...
  private async closeAnyOpenModal(): Promise<void> {
    try {
      ...
      if (
        (await this.page.locator(overlaySelector).count()) > 0 ||
  ...
  async cleanupAll(): Promise<void> {
    ...
    // タイプ別にグループ化
    const groupedData = this.createdData.reduce(
      (acc, data) => {
    ...
    for (const [type, dataList] of Object.entries(groupedData)) {
      await this.cleanupByType(type as 'teachers' | 'subjects' | 'classrooms', dataList)
    }
        ...
        try {
          ...
          if ((await deleteButton.count()) > 0) {
            ...
            const confirmButton = this.page.locator(
              'button:has-text("削除"), button:has-text("Delete"), button:has-text("確認")'
            )
   ...
   */
  async cleanupByPattern(
    patterns: { type: 'teacher' | 'subject' | 'classroom'; pattern: RegExp }[]
  ): Promise<void> {
   ...
   */
  private async cleanupByPatternForType(
    type: 'teacher' | 'subject' | 'classroom',
    pattern: RegExp
  ) {
    try {
  ...
  getCleanupSummary(): string {
    const summary = this.createdData.reduce(
      (acc, data) => {
    ...
    return `テストデータ記録: ${Object.entries(summary)
      .map(([type, count]) => `${type}=${count}`)
      .join(', ')} (合計${this.createdData.length}件)`
  }
}
