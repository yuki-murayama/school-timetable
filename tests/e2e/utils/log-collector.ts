...
export class LogCollector {
  ...
  private setupLogCollection() {
    ...
    this.page.on('response', async response => {
      ...
      if (url.includes('/api/') || url.includes('/frontend/')) {
        let responseBody: unknown = 'Could not read body'
        try {
  ...
  printLogs() {
    ...
    console.log(
      `📊 Statistics: ${stats.total} total, ${stats.errors} errors, ${stats.warnings} warnings, ${stats.apiRequests} API requests`
    )
      ...
      this.getApiLogs().forEach((log, index) => {
        const statusEmoji =
          log.status && log.status >= 400
            ...
            : log.status && log.status >= 200 && log.status < 300
              ...
              : '❓'
        console.log(
          `${index + 1}. ${statusEmoji} [${log.timestamp}] ${log.message} (${log.status || 'No status'})`
        )
        ...
        if (log.details?.responseBody) {
          ...
          if (typeof body === 'string' && body.includes('<!DOCTYPE html>')) {
            ...
            console.log(
              `   📄 JSON Response: ${JSON.stringify(body, null, 2).substring(0, 200)}...`
            )
          }
        }
      })
  ...
  // カスタムログエントリを追加
  addCustomLog(level: 'info' | 'warn' | 'error' | 'debug', message: string, details?: unknown) {
    this.logs.push({
      timestamp: new Date().toISOString(),
      type: 'console',
      level,
