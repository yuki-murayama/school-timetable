...
test.describe('本番環境エラーデバッグ', () => {
  ...
  test('本番環境データ登録画面エラーの詳細調査', async ({ page }) => {
    ...
    // 詳細なネットワーク監視
    const networkLogs: Array<{method: string, url: string, status: number, response?: any}> = []
    ...
    try {
      ...
      // データ登録ボタンを詳細に調査
      const dataButtons = await page.locator('button:has-text("データ登録"), a:has-text("データ登録")').all()
      ...
      // タブ要素の詳細調査
      const tabs = await page.locator('[role="tab"], button:contains("基本設定"), button:contains("設定")').all()
      ...
      // 基本設定タブをクリック
      const basicTab = page.locator('button:has-text("基本設定"), [role="tab"]:has-text("基本設定")')
      ...
      // コンソールログの詳細分析
      console.log('\n💬 コンソールログ分析:')
      const errorConsoleLogs = consoleLogs.filter(log => 
        log.type === 'error' || 
      ...
      const hasServerError = networkLogs.some(log => 
        log.status >= 500 || 
        (log.response && JSON.stringify(log.response).includes('Internal Server Error'))
      )
      
      const hasSettingsError = consoleLogs.some(log => 
