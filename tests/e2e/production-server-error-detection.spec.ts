...
test.describe('本番環境サーバーエラー検知', () => {
  test('データ登録画面（基本情報）でのサーバー内部エラー検知テスト', async ({ page }) => {
    ...
    page.on('console', msg => {
      ...
      if (msg.type() === 'error') {
        ...
        text.includes('設定読み込みエラー') ||
        text.includes('バックエンドAPI') ||
        text.includes('500エラー')
      ) {
    ...
    try {
      ...
      // 6. 特定のエラーパターンの確認
      const hasTokenError = errorReport.consoleErrors.some(
        error =>
      ...
      const hasServerError = errorReport.networkErrors.some(
        error => error.includes('500') || error.includes('Internal Server Error')
      )

      const hasSettingsError = errorReport.consoleErrors.some(
        error =>
          error.includes('設定読み込みエラー') || error.includes('バックエンドAPIが500エラー')
      )
      ...
      if (hasTokenError || hasServerError || hasSettingsError) {
        ...
        // 検知されたエラーの詳細表示
        if (hasTokenError) {
          const tokenErrors = errorReport.consoleErrors.filter(
            error =>
              error.includes('Invalid or expired token') ||
              error.includes('Token validation failed') ||
              error.includes('Authentication failed')
        ...
        if (hasServerError) {
          const serverErrors = errorReport.networkErrors.filter(
            error => error.includes('500') || error.includes('Internal Server Error')
          )
        ...
        if (hasSettingsError) {
          const settingsErrors = errorReport.consoleErrors.filter(
            error =>
              error.includes('設定読み込みエラー') || error.includes('バックエンドAPIが500エラー')
          )
  ...
  test('API直接アクセスによるサーバー内部エラー検知テスト', async ({ page }) => {
    ...
    console.log(
      '📋 API検証完了。エラーレポート:',
      JSON.stringify(errorReport.totalErrors > 0 ? errorReport : '正常')
    )
  })
})
