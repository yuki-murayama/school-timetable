...
test.describe('🔄 完全統合ワークフローE2Eテスト', () => {
  ...
  test('ログインから時間割生成まで完全ワークフロー', async ({ page }) => {
    ...
    for (const { name, value } of schoolSettings) {
      const input = page.locator(`input[name="${name}"], input[name*="${name.replace('grade', 'grade')}"]`).first()
    ...
    for (let i = 0; i < testData.teachers.length; i++) {
      ...
      // 教師追加ボタンをクリック
      const addTeacherButton = page.locator('button:has-text("教師を追加"), button:has-text("追加")').first()
      ...
      for (const subject of teacher.subjects) {
        const subjectCheckbox = page.locator(`input[value="${subject}"], checkbox:has-text("${subject}")`).first()
      ...
      for (const grade of teacher.grades) {
        const gradeCheckbox = page.locator(`input[value="${grade}"], checkbox:has-text("${grade}年")`).first()
    ...
    for (const subject of testData.subjects) {
      ...
      const addSubjectButton = page.locator('button:has-text("教科を追加"), button:has-text("追加")').first()
      if (await addSubjectButton.count() > 0) {
        await addSubjectButton.click()
        await page.waitForTimeout(1000)
        
        const subjectNameInput = page.locator('input[name="name"], input[placeholder*="教科"]').first()
    ...
    for (const classroom of testData.classrooms) {
      ...
      const addClassroomButton = page.locator('button:has-text("教室を追加"), button:has-text("追加")').first()
      if (await addClassroomButton.count() > 0) {
        ...
        const classroomNameInput = page.locator('#classroom-name, input[id="classroom-name"]').first()
        if (await classroomNameInput.count() > 0) {
          await classroomNameInput.fill(classroom.name)
          
          const saveClassroomButton = page.locator('button:has-text("保存"), button:has-text("追加")').first()
    ...
    // 時間割生成実行
    const generateButton = page.locator('button:has-text("生成開始"), button:has-text("生成"), button:has-text("Generate")').first()
    ...
    // 時間割表示画面に遷移
    const viewButton = page.locator('button:has-text("時間割を見る"), button:has-text("表示"), a:has-text("時間割表示")').first()
    ...
    if (await timetableDisplay.count() > 0) {
      ...
      const subjectCells = page.locator('td:has-text("数学"), td:has-text("国語"), td:has-text("英語")')
    ...
    const functionalityChecks = [
      ...
      { name: 'データ登録リンク', selector: 'button:has-text("データ登録"), a:has-text("データ登録")' },
      { name: '時間割生成リンク', selector: 'button:has-text("時間割生成"), a:has-text("時間割生成")' }
    ]
  ...
  test('エラーハンドリングと異常系の動作確認', async ({ page }) => {
    ...
    if (await timetableButton.count() > 0) {
      ...
      if (await generateButton.count() > 0) {
        ...
        if (isDisabled) {
          ...
          const errorMessages = page.locator('text="エラー", text="不足", text="Error", .error, [role="alert"]')
    ...
    if (await dataButton.count() > 0) {
      ...
      if (await basicTab.count() > 0) {
        ...
        const gradeInput = page.locator('input[name="grade1Classes"], input[name*="grade1"]').first()
        if (await gradeInput.count() > 0) {
          ...
          if (await saveButton.count() > 0) {
            ...
            // バリデーションエラーの確認
            const validationErrors = page.locator('text*="無効", text*="エラー", .error, [role="alert"]')
