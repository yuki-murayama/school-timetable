...
test.describe('セッション切れ時のログイン強制移動テスト', () => {
  ...
  test('意図的なセッション切れでログインページに自動移動', async ({ page }) => {
    ...
    // 基本設定タブをクリック
    await page.click(
      'button[data-state="active"]:has-text("基本設定"), button:has-text("基本設定")'
    )
    ...
    const sessionExpiredMessage = page.locator(
      'text=/セッション.*期限切れ|ログイン.*必要|再.*ログイン/'
    )
  ...
  test('別の画面でのセッション切れテスト - 教師管理画面', async ({ page }) => {
    ...
    await page.waitForSelector('h2:has-text("教師一覧"), h3:has-text("教師一覧")', {
      timeout: 10000,
    })
  ...
  test('認証が必要な複数画面での一貫したセッション切れ処理', async ({ page }) => {
    ...
    const testScreens = [
      { name: '基本設定', tab: '基本設定', action: 'button:has-text("設定を保存")' },
      {
        name: '教師情報',
        tab: '教師情報',
        action: 'button:has-text("教師を追加"), button:has-text("追加")',
      },
      {
        name: '教科情報',
        tab: '教科情報',
        action: 'button:has-text("教科を追加"), button:has-text("追加")',
      },
    ]
