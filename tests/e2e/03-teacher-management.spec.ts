...
import { createErrorMonitor, ErrorMonitor } from './utils/error-monitor'
...
test.describe('👨‍🏫 教師管理E2Eテスト', () => {
  ...
  test('教師のドラッグ&ドロップ順序変更と永続化確認', async ({ page }) => {
    ...
    const hasEmptyMessage = await page.locator('tbody tr td:has-text("教師情報が登録されていません")').count() > 0
    ...
    const teacherTabAfterReload = page.locator('button:has-text("教師情報"), button:has-text("教師")').first()
