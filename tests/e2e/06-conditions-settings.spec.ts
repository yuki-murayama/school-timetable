...
test.describe('⚙️ 条件設定E2Eテスト', () => {
  ...
  test('条件設定の初期値確認と更新保存の一連の流れ', async ({ page }) => {
    ...
    // 条件設定カードの確認
    const conditionsCard = page.locator('h3:has-text("任意条件設定"), h2:has-text("条件設定"), .card')
    ...
    // 条件設定タブを再度選択
    const conditionsTabAfterReload = page.locator('button:has-text("条件設定"), [role="tab"]:has-text("条件設定")').first()
  ...
  test('条件設定画面の初期表示確認', async ({ page }) => {
    ...
    const conditionsTab = page.locator('button:has-text("条件設定"), button:has-text("条件")').first()
    ...
    // 保存ボタンの確認
    const saveButton = page.locator('button:has-text("保存"), button:has-text("条件設定を保存")').first()
