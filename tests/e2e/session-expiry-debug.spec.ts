...
test.describe('セッション切れデバッグテスト', () => {
  test('データ管理画面の構造確認とセッション切れテスト', async ({ page }) => {
    ...
    const baseURL = process.env.E2E_BASE_URL || process.env.PLAYWRIGHT_BASE_URL || 'https://school-timetable-monorepo.grundhunter.workers.dev'
    ...
    // タブ要素を確認
    const tabs = await page.locator('[role="tab"], button:contains("設定"), button:contains("情報"), button:contains("基本")').allTextContents()
    ...
    const isDataManagementPage = headings.some(h => h.includes('データ') || h.includes('設定') || h.includes('管理')) ||
                                  tabs.some(t => t.includes('基本') || t.includes('教師') || t.includes('教科'))
    
    if (isDataManagementPage) {
      console.log('✅ データ管理画面確認完了')
    } else {
      console.log('⚠️ データ管理画面の確認ができませんでした。スクリーンショットを確認してください。')
    ...
    try {
      // 基本設定タブを探してクリック
      const basicSettingsTab = page.locator('button:contains("基本"), [role="tab"]:contains("基本")')
