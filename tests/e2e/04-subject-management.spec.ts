...
test.describe('📚 教科管理E2Eテスト', () => {
  ...
  test('教科の新規追加・編集・削除の一連の流れ', async ({ page }) => {
    ...
    if (await createdSubjectRow.count() > 0) {
      ...
      // 編集ボタンを特定してクリック
      const editButton = createdSubjectRow.locator('button[aria-label*="編集"], button[data-testid*="edit"], button:has-text("編集")')
      if (await editButton.count() > 0) {
        ...
        const nameInput = page.locator('#subject-name, input[name="name"], input[placeholder*="教科名"]').first()
        if (await nameInput.count() > 0) {
          ...
          // 保存
          const updateButton = page.locator('button:has-text("保存"), button:has-text("更新")').first()
    ...
    if (await subjectRowToDelete.count() > 0) {
      ...
      // 削除ボタンを探してクリック
      const deleteButton = subjectRowToDelete.locator('button[aria-label*="削除"], button[data-testid*="delete"], button:has([data-testid*="trash"])')
      if (await deleteButton.count() > 0) {
        await deleteButton.first().click({ force: true })
        await page.waitForTimeout(1000)
        
        // 削除確認ダイアログがある場合は確認
        const confirmButton = page.locator('button:has-text("削除"), button:has-text("確認"), button:has-text("はい")').first()
      ...
      if (await originalSubjectRow.count() > 0) {
        const deleteButton = originalSubjectRow.locator('button[aria-label*="削除"], button[data-testid*="delete"], button:has([data-testid*="trash"])')
        if (await deleteButton.count() > 0) {
          await deleteButton.first().click({ force: true })
          await page.waitForTimeout(1000)
  ...
  test('教科のドラッグ&ドロップ順序変更と永続化確認', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(rowCount, 5); i++) { // 最大5件まで確認
