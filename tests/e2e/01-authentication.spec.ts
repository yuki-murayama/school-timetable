...
test.describe('🔐 認証機能E2Eテスト', () => {
  ...
  test('ログイン・ログアウトの一連の流れ', async ({ page }) => {
    ...
    const signInButton = page.locator('button:has-text("Sign in"), button:has-text("ログイン"), button:has-text("Login")')
    ...
    // メールアドレス入力
    const emailInput = page.locator('input[name="email"], input[type="email"], input[placeholder*="email"], input[placeholder*="メール"]')
    await expect(emailInput.first()).toBeVisible({ timeout: 10000 })
    await emailInput.first().fill(E2E_TEST_USER.email)
    console.log(`📧 メールアドレス入力完了: ${E2E_TEST_USER.email}`)
    
    // パスワード入力
    const passwordInput = page.locator('input[name="password"], input[type="password"], input[placeholder*="password"], input[placeholder*="パスワード"]')
    await expect(passwordInput.first()).toBeVisible({ timeout: 5000 })
    await passwordInput.first().fill(E2E_TEST_USER.password)
    console.log('🔑 パスワード入力完了')
    
    // ログインボタンクリック
    const submitButton = page.locator('button[type="submit"], button:has-text("Continue"), button:has-text("ログイン"), button:has-text("Login")')
    ...
    for (const selector of logoutElements) {
      ...
      if (await element.count() > 0) {
        ...
        const backToLogin = await page.locator('button:has-text("Sign in"), button:has-text("ログイン"), input[type="email"]').count() > 0
        if (backToLogin) {
  ...
  test('無効な認証情報でのログイン失敗確認', async ({ page }) => {
    ...
    if (await emailInput.count() > 0) {
      ...
      if (await passwordInput.count() > 0) {
        ...
        // ログインボタンクリック
        const submitButton = page.locator('button[type="submit"], button:has-text("Continue"), button:has-text("ログイン")')
