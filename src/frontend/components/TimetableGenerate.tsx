...
import { Wand2, Settings, Loader2, CheckCircle } from "lucide-react"
...
export function TimetableGenerate() {
  ...
  const handleProgramGenerate = async () => {
    ...
    try {
      ...
      if (result && result.timetable && result.statistics) {
  ...
  const handleLegacyGenerate = async () => {
    ...
    try {
      ...
      const result = await timetableApi.generateTimetable({ options }, { token: token || undefined })
      ...
      <Card>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                <Switch id="legacy-toggle" checked={useLegacyGeneration} onCheckedChange={setUseLegacyGeneration} />
              </div>
              <div className="flex items-center space-x-2">
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card 
                className={`cursor-pointer transition-colors ${
                  generationMethod === 'program' ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-gray-50'
                }`}
              ...
              <Card 
                className={`cursor-pointer transition-colors ${
                  generationMethod === 'ai' ? 'ring-2 ring-green-500 bg-green-50' : 'hover:bg-gray-50'
                }`}
          ...
          <div className="flex justify-end space-x-4 pt-4">
            <Button variant="outline" disabled={isGenerating || isGeneratingProgram}>条件をリセット</Button>
      ...
      {!useLegacyGeneration && isGenerating && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>時間割を段階的に生成しています。3-5分程度かかる場合があります。</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold">{generationResult.data?.metadata?.statistics?.generationTime || '不明'}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-muted-foreground">割当数</p>
                  <p className="text-2xl font-bold">{generationResult.data?.metadata?.statistics?.totalAssignments || 0}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-muted-foreground">制約違反</p>
                  <p className="text-2xl font-bold">{generationResult.data?.metadata?.statistics?.constraintViolations || 0}</p>
                </div>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-green-800">
                  <strong>生成日時:</strong> {new Date(generationResult.data?.created_at || Date.now()).toLocaleString('ja-JP')}
                </p>
                <p className="text-sm text-green-800 mt-2">
