...
import { teacherApi, type Teacher, type AssignmentRestriction } from "../../lib/api"
...
export const TeachersSection = memo(function TeachersSection({
  ...
  }, [token, getFreshToken, teachers, onTeachersUpdate, toast])
        ...
        timeoutRef.current = setTimeout(async () => {
          if (token) {
            try {
              ...
              const reorderData = itemsWithOrder.map((teacher, index) => ({
                id: teacher.id || '',
                order: index
              })).filter(item => item.id)
              
              const result = await teacherApi.reorderTeachers(reorderData, { token, getFreshToken })
  ...
  }, [teachers, onTeachersUpdate, token, getFreshToken, toast])
    ...
    <>
              ...
              <Table>
                ...
                <TableBody>
                  {!Array.isArray(teachers) || teachers.length === 0 ? (
                    ...
                    <SortableContext items={sortedTeachers.map(t => t.id || '')} strategy={verticalListSortingStrategy}>
                            ...
                            <SortableRow key={teacher.id} id={teacher.id || ''}>
                                  ...
                                  {(() => {
                                    try {
                                      ...
                                      if (subjectsArray.length === 0) {
                                        ...
                                        return <span className="text-gray-400 text-xs">科目なし</span>
                                      }
                                      ...
                                      console.error(`Error rendering subjects for ${teacher.name}:`, error)
                              ...
                              <TableCell>
                                <div className="flex flex-wrap gap-1">
                                  {(() => {
                                    try {
                                      return (teacher.grades || []).map((grade, index) => (
                                        <Badge key={index} variant="outline">
                                          {typeof grade === 'string' ? grade : grade.name || grade}
                                        </Badge>
                                      ))
                                    } catch (error) {
                                      console.error(`Error rendering grades for ${teacher.name}:`, error)
                                      return <span className="text-red-400 text-xs">エラー</span>
                                    }
                                  })()}
                                </div>
                                ...
                                <div className="text-sm">
                                  {(() => {
                                    try {
                                      ...
                                      return <span className="text-blue-600 font-medium">割当制限あり</span>
                                    } catch (error) {
                                      console.error(`Error rendering restrictions for ${teacher.name}:`, error)
                                      return <span className="text-red-400 text-xs">エラー</span>
                                    }
                                  })()}
                                </div>
      ...
      <TeacherEditDialog
        ...
        onSave={(updatedTeacher) => {
          console.log('🔄 Processing updated teacher from dialog:', JSON.stringify(updatedTeacher, null, 2))
