...
import { subjectApi, type Subject } from "../../lib/api"
...
export function SubjectsSection({ token, getFreshToken }: SubjectsSectionProps) {
  ...
  console.log('SubjectsSection render - subjects:', subjects, 'is array:', Array.isArray(subjects), 'loading:', isSubjectsLoading, 'count:', subjects?.length || 0)

  // Helper function to format target grades for display
  const formatGrades = (targetGrades?: number[] | any) => {
    console.log('🎯 formatGrades called with:', targetGrades, 'isArray:', Array.isArray(targetGrades), 'length:', targetGrades?.length)
  ...
  useEffect(() => {
    const loadSubjects = async () => {
      console.log('loadSubjects called, token:', !!token, 'getFreshToken:', !!getFreshToken)
      ...
      const timeoutId = setTimeout(() => {
        ...
        toast({
          title: "読み込みタイムアウト",
          description: "教科情報の読み込みに時間がかかりすぎています。ページを再読み込みしてください。",
          variant: "destructive",
        })
      }, 15000) // 15秒でタイムアウト
      ...
      try {
        ...
        console.log('Setting subjects loading to false')
        setIsSubjectsLoading(false)
      }
    }
  ...
  const handleDeleteSubject = async (id: string) => {
    ...
    try {
      ...
      if (error?.message?.includes('404') || error?.response?.status === 404) {
        ...
        setSubjects((prevSubjects) => (Array.isArray(prevSubjects) ? prevSubjects : []).filter(s => s.id !== id))
        toast({
          title: "削除完了",
  ...
  const handleSaveSubject = async (subjectData: Partial<Subject>) => {
    ...
    try {
      if (editingSubject?.id) {
        ...
        const updatedSubject = await subjectApi.updateSubject(editingSubject.id, subjectData, { token, getFreshToken })
        console.log('✅ API returned updated subject:', updatedSubject, 'targetGrades:', updatedSubject.targetGrades)
        
        setSubjects((prevSubjects) => {
          const newSubjectsList = (Array.isArray(prevSubjects) ? prevSubjects : []).map(s => {
            if (s.id === editingSubject.id) {
              console.log('🔄 Replacing subject:', s.id, 'old targetGrades:', s.targetGrades, 'new targetGrades:', updatedSubject.targetGrades)
              return updatedSubject
            }
            return s
          })
          console.log('📊 Updated subjects list:', newSubjectsList.map(s => ({ id: s.id, name: s.name, targetGrades: s.targetGrades })))
        ...
        console.log('✅ API returned new subject:', newSubject, 'targetGrades:', newSubject.targetGrades)
        setSubjects((prevSubjects) => [...(Array.isArray(prevSubjects) ? prevSubjects : []), newSubject])
        toast({
          title: "追加完了",
      ...
      setSubjects((items) => {
        ...
        if (token) {
          ...
          orderUpdateTimeoutRef.current = setTimeout(() => {
            subjectApi.reorderSubjects(orderData, { token, getFreshToken }).then((result) => {
                  ...
                  {!Array.isArray(subjects) || subjects.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center text-gray-500 py-8">
                        {!Array.isArray(subjects) ? '教科情報の読み込み中にエラーが発生しました' : '教科情報が登録されていません'}
                      </TableCell>
                    </TableRow>
                  ) : (
