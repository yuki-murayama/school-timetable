...
import { teacherApi, subjectApi, schoolApi, type Teacher, type Subject, type AssignmentRestriction, type SchoolSettings } from "../../lib/api"
...
interface SortableRestrictionItemProps {
  ...
  index: number
  onUpdate: (index: number, field: keyof AssignmentRestriction, value: any) => void
...
function SortableRestrictionItem({
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: `restriction-${index}` })
    ...
    >
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          {Array.from({ length: getPeriodsForDay(restriction.restrictedDay) }, (_, i) => i + 1).map(period => (
...
export function TeacherEditDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      if (teacher?.id) {
        ...
        const updatedTeacher = await teacherApi.updateTeacher(teacher.id, formData, { token, getFreshToken })
  ...
  const handleAddRestriction = () => {
    ...
    if (formData.assignmentRestrictions.length > 0) {
      // 表示順序でソートして最後の制限を取得
      const sortedRestrictions = [...formData.assignmentRestrictions].sort((a, b) => a.displayOrder - b.displayOrder)
  ...
  const handleRemoveRestriction = (index: number) => {
    setFormData(prev => ({
      ...prev,
      assignmentRestrictions: prev.assignmentRestrictions.filter((_, i) => i !== index)
  ...
  const handleUpdateRestriction = (index: number, field: keyof AssignmentRestriction, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent 
        side="right"
        className="sm:max-w-[700px] w-full overflow-y-auto"
      >
        <SheetHeader>
          <SheetTitle>
            {teacher ? "教師情報を編集" : "新しい教師を追加"}
          </SheetTitle>
          <SheetDescription>
            ...
            {formData.assignmentRestrictions.length === 0 ? (
              ...
              >
                <div className="space-y-4 max-h-60 overflow-y-auto">
                  <SortableContext
                    items={formData.assignmentRestrictions.map((_, index) => `restriction-${index}`)}
