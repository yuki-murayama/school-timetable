  ...
  const handleSaveClassroom = async (classroomData: Partial<Classroom>) => {
    ...
    try {
      if (editingClassroom?.id) {
        // Update
        const updatedClassroom = await classroomApi.updateClassroom(editingClassroom.id, classroomData, { token, getFreshToken })
        ...
        // Create new
        const newClassroom = await classroomApi.createClassroom(classroomData, { token, getFreshToken })
        ...
        if (token) {
          timeoutRef.current = setTimeout(async () => {
            try {
              ...
              const result = await classroomApi.reorderClassrooms(reorderData, { token, getFreshToken })
              
              console.log('✅ 教室順序一括更新完了:', result.updatedCount, '/', result.totalRequested)
                    ...
                    <SortableContext items={classrooms.map(c => c.id || '')} strategy={verticalListSortingStrategy}>
                      {classrooms.map((classroom) => (
                        <SortableRow key={classroom.id} id={classroom.id || ''}>
                          ...
                          <TableCell>
                            <span className="text-sm">{classroom.count || classroom.capacity || 1}室</span>
                          </TableCell>
                          <TableCell>
