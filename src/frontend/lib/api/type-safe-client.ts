...
export const ApiClientStats = {
  recordRequest(success: boolean, responseTimeMs: number, error?: unknown): void {
    apiClientStats.totalRequests++

    if (success) {
      apiClientStats.successfulRequests++
    } else {
      apiClientStats.failedRequests++
      apiClientStats.lastError = error

      if (error instanceof ValidationError) {
        apiClientStats.validationErrors++
      } else {
        apiClientStats.networkErrors++
      }
    }

    // 移動平均計算
    apiClientStats.averageResponseTime =
      (apiClientStats.averageResponseTime * (apiClientStats.totalRequests - 1) + responseTimeMs) /
      apiClientStats.totalRequests
  },
