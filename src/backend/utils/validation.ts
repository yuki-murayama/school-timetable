    ...
    if (timetable[day] && Array.isArray(timetable[day])) {
      ...
      for (const period of daySchedule) {
        ...
        if (!isPartialGeneration && period.teacher) {
          ...
          const sameTimeSlots = daySchedule.filter(
            p => p.period === period.period && p.teacher === period.teacher
          )
        ...
        // 教室の重複チェック
        if (period.classroom) {
          const sameClassroom = daySchedule.filter(
            p => p.period === period.period && p.classroom === period.classroom
          )
    ...
    if (timetable[day] && Array.isArray(timetable[day])) {
      for (const period of timetable[day]) {
        if (period.teacher && !teacherNames.includes(period.teacher)) {
          errors.push(
            `${day} 時限${period.period}: 未登録の教師 ${period.teacher} が使用されています`
          )
        }

        if (period.subject && !subjectNames.includes(period.subject)) {
          errors.push(
            `${day} 時限${period.period}: 未登録の科目 ${period.subject} が使用されています`
          )
        }

        if (period.classroom && !classroomNames.includes(period.classroom)) {
          errors.push(
            `${day} 時限${period.period}: 未登録の教室 ${period.classroom} が使用されています`
          )
        }
      }
    }
