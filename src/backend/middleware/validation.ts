 ...
 * 数値バリデーション
 */
export function validateNumber(input: any, min: number = 0, max: number = 999999): number {
          ...
          switch (rules.type) {
            case 'string':
              ...
              if (rules.minLength && validatedBody[key].length < rules.minLength) {
                throw new HTTPException(400, { message: `Field '${key}' must be at least ${rules.minLength} characters` })
              }
              if (rules.maxLength && validatedBody[key].length > rules.maxLength) {
                throw new HTTPException(400, { message: `Field '${key}' must be at most ${rules.maxLength} characters` })
              }
              break
            ...
            case 'email':
              ...
              if (!validateEmail(sanitizedEmail)) {
                throw new HTTPException(400, { message: `Field '${key}' must be a valid email address` })
              }
            ...
            case 'array':
              ...
              validatedBody[key] = value.map((item: any) => 
                typeof item === 'string' ? sanitizeString(item) : item
              )
              if (rules.maxItems && validatedBody[key].length > rules.maxItems) {
                throw new HTTPException(400, { message: `Field '${key}' must have at most ${rules.maxItems} items` })
              }
              break
 ...
 */
export function validateSqlParameters(params: any[]): any[] {
