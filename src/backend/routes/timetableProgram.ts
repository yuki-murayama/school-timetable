...
import type { Classroom, Env, SchoolSettings, Subject, Teacher } from '../../shared/types'
...
app.post('/generate', async c => {
  try {
    ...
    const teachers: Teacher[] = (teachersResult.results || []).map(
      (t: DatabaseTeacher): Teacher => {
    ...
    const subjects: Subject[] = (subjectsResult.results || []).map(
      (s: DatabaseSubject): Subject => {
        console.log('🔍 教科データ変換:', {
          name: s.name,
    ...
    const classrooms: Classroom[] = (classroomsResult.results || []).map(
    ...
    try {
      ...
      if (useOptimization) {
        ...
        console.log(
          `✅ 教科拡張完了: 拡張対象${processedSubjects.filter(s => s.grades && s.grades.length > 0).length}科目`
        )
      }
      ...
      if (useOptimization) {
        ...
        let bestResult: TimetableValidationResult | null = null
        let bestRate = 0
        const maxRetries = 5
        ...
        for (let retry = 0; retry < maxRetries; retry++) {
          ...
          if (retry < maxRetries - 1) {
            const newGenerator = new TimetableGenerator(
              settings,
              teachers,
              processedSubjects,
              classrooms,
      ...
      return c.json(
        {
          ...
          error:
            generatorError instanceof Error ? generatorError.message : 'TimetableGenerator error',
        },
        500
      )
    }
...
app.get('/config', async c => {
  try {
    ...
    // 割当制限統計
    const restrictionsStats = await db
      .prepare(`
      SELECT 
...
app.get('/debug-data', async c => {
  try {
    ...
    // 変換後のデータ形式を確認
    const teachers = (teachersResult.results || []).slice(0, 3).map((t: DatabaseTeacher) => {
    ...
    const subjects = (subjectsResult.results || []).slice(0, 3).map((s: DatabaseSubject) => {
...
app.get('/quick-test', async c => {
  try {
      ...
      settings = {
        ...
        grades:
          typeof rawSettings.grades === 'string'
            ...
            : rawSettings.grades,
        classesPerGrade:
          typeof rawSettings.classesPerGrade === 'string'
            ? JSON.parse(rawSettings.classesPerGrade)
            : rawSettings.classesPerGrade,
      }
    } else {
      // デフォルト設定
      settings = {
    ...
    const teachers =
      teachersResult.results?.slice(0, 5).map(teacher => ({
        ...teacher,
        subjects:
          typeof teacher.subjects === 'string' ? JSON.parse(teacher.subjects) : teacher.subjects,
      })) || []
    ...
    const subjects =
      subjectsResult.results?.slice(0, 5).map(subject => {
        let grades = []
        try {
        ...
        if (!weeklyHours || weeklyHours <= 0) {
          ...
          console.log(
            `⚠️ 教科「${subject.name}」のweeklyHours空値をデフォルト値に設定:`,
            weeklyHours
          )
        }
      ...
      for (const subject of subjects) {
        ...
        testResult.canTeach =
          teacher.subjects?.some(
            s =>
              s === subject.name ||
              (typeof s === 'object' && (s.id === subject.id || s.name === subject.name))
          ) || false
...
app.get('/constraint-analysis', async c => {
  try {
    ...
    // 時間割生成を実行して制約状況を分析
    try {
      await generator.generateTimetable({ tolerantMode: false })
    } catch (_error) {
...
app.post('/validate', async c => {
  try {
    ...
    // 教師データを取得
    const teachersResult = await db.prepare('SELECT * FROM teachers ORDER BY name').all()
    const teachers: Teacher[] = (teachersResult.results || []).map((t: DatabaseTeacher) => {
      let subjects = []
      if (t.subjects) {
        try {
      ...
      if (t.assignment_restrictions) {
        try {
          assignmentRestrictions =
            typeof t.assignment_restrictions === 'string'
              ? JSON.parse(t.assignment_restrictions)
              : t.assignment_restrictions
        } catch (e) {
    ...
    // 教科データを取得
    const subjectsResult = await db.prepare('SELECT * FROM subjects ORDER BY name').all()
    const subjects: Subject[] = (subjectsResult.results || []).map((s: DatabaseSubject) => {
      let grades = [1, 2, 3]
      if (s.target_grades) {
        try {
          if (typeof s.target_grades === 'string') {
    ...
    console.log(
      `📊 検証用データ準備完了: 教師${teachers.length}名, 教科${subjects.length}科目, 教室${classrooms.length}室`
    )
    ...
    return c.json({
      ...
      data: {
        ...
        summary: {
          ...
          criticalViolations: validationResult.violations.filter(v => v.severity === 'critical')
            .length,
          ...
          unassignedHours: validationResult.unassignedRequirements.reduce(
            (sum, req) => sum + req.missingHours,
            0
          ),
...
app.get('/validate-light', async c => {
  try {
    ...
    const classroomsResult = await db
      .prepare('SELECT * FROM classrooms ORDER BY name LIMIT 3')
      .all()
    const classrooms: Classroom[] = (classroomsResult.results || []).map(
      (c: DatabaseClassroom) => ({
        id: c.id,
    ...
    console.log(
      `📊 軽量検証用データ準備完了: 教師${teachers.length}名, 教科${subjects.length}科目, 教室${classrooms.length}室`
    )
...
app.post('/generate-optimized', async c => {
  ...
  try {
    ...
    console.log(
      `📊 最適化用データ準備完了: 教師${teachers.length}名, 教科${subjects.length}科目, 教室${classrooms.length}室`
    )

    // 最適化TimetableGeneratorを初期化（パフォーマンス向上のためデバッグ無効）
    const _generator = new TimetableGenerator(settings, teachers, subjects, classrooms, false)
    ...
    const optimizedGenerator = new TimetableGenerator(
      settings,
      processedTeachers,
      expandedSubjects,
      classrooms,
    ...
    for (let retry = 0; retry < maxRetries; retry++) {
      ...
      const attemptResult = await optimizedGenerator.generateTimetable({ tolerantMode: true })
      if (attemptResult.statistics) {
        const rate =
          (attemptResult.statistics.assignedSlots / attemptResult.statistics.totalSlots) * 100
    ...
    const assignmentRate = result.statistics
      ? ((result.statistics.assignedSlots / result.statistics.totalSlots) * 100).toFixed(1)
      : 0
    ...
    return c.json({
      ...
      data: {
        ...
        result: {
          ...
          bestAssignmentRate: result.statistics?.bestAssignmentRate || 0,
          improvement:
            parseFloat(assignmentRate) > 35.3
              ? `+${(parseFloat(assignmentRate) - 35.3).toFixed(1)}%`
              : 'N/A',
        },
...
app.post('/save', async c => {
  try {
    ...
    console.log(
      `📊 保存統計: 割当率${statistics.bestAssignmentRate || 0}%, 総スロット${statistics.totalSlots || 0}`
    )
...
app.get('/saved', async c => {
  try {
    ...
    const timetables = (result.results || []).map((row: TimetableRow) => ({
      id: row.id,
