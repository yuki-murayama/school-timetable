...
import type { SchoolSettings, Teacher, Subject, Classroom, Env } from '../../shared/types'
import { 
  clerkAuthMiddleware, 
  adminAuthMiddleware, 
...
schoolRoutes.get('/teachers', clerkAuthMiddleware, readOnlyAuthMiddleware, async (c) => {
  try {
    ...
    let hasAssignmentRestrictionsColumn = tableInfo.results?.some(col => col.name === 'assignment_restrictions')
    ...
    if (!hasAssignmentRestrictionsColumn) {
      try {
        ...
        await db.prepare(`ALTER TABLE teachers ADD COLUMN assignment_restrictions TEXT DEFAULT '[]'`).run()
...
schoolRoutes.post('/teachers', 
  ...
  try {
    ...
    let hasAssignmentRestrictionsColumnForCreate = createTableInfo.results?.some(col => col.name === 'assignment_restrictions')
    ...
    console.log('🔍 作成時assignment_restrictionsカラム存在:', hasAssignmentRestrictionsColumnForCreate)
    ...
    if (hasGradesColumnForCreate && hasAssignmentRestrictionsColumnForCreate && hasSubjectsColumnForCreate) {
      await db.prepare(`
    ...
    if (body.subjects && Array.isArray(body.subjects) && body.subjects.length > 0) {
      for (const subjectName of body.subjects) {
        ...
        // 科目名から科目IDを取得
        const subject = await db.prepare('SELECT id FROM subjects WHERE name = ?').bind(subjectName).first()
        ...
        if (subject) {
          ...
          `).bind(relationId, teacherId, subject.id).run()
    ...
    const hasAssignmentRestrictionsColumn = tableInfo.results?.some(col => col.name === 'assignment_restrictions')
    ...
    // assignment_restrictionsフィールドをパース
    if (newTeacher.assignment_restrictions && typeof newTeacher.assignment_restrictions === 'string') {
      try {
...
schoolRoutes.put('/teachers/:id', 
  ...
  try {
    ...
    // 教師の存在確認
    const existing = await db.prepare('SELECT id FROM teachers WHERE id = ?').bind(teacherId).first()
    if (!existing) {
      return c.json({
        success: false,
    ...
    if (body.grades !== undefined) {
      try {
        ...
        await db.prepare('SELECT grades FROM teachers WHERE id = ? LIMIT 1').bind(teacherId).first()
        updateFields.push('grades = ?')
        updateValues.push(JSON.stringify(body.grades))
      } catch (error) {
        if (error.message && (error.message.includes('no such column: grades') || error.message.includes('has no column named grades'))) {
    ...
    if (body.assignmentRestrictions !== undefined) {
      try {
        ...
        await db.prepare('SELECT assignment_restrictions FROM teachers WHERE id = ? LIMIT 1').bind(teacherId).first()
        ...
        if (error.message && (error.message.includes('no such column: assignment_restrictions') || error.message.includes('has no column named assignment_restrictions'))) {
          console.log('assignment_restrictionsカラムが存在しないため、assignment_restrictions更新をスキップします')
        } else {
          throw error
        }
      }
    ...
    if (body.subjects !== undefined) {
      try {
        ...
        await db.prepare('SELECT subjects FROM teachers WHERE id = ? LIMIT 1').bind(teacherId).first()
        updateFields.push('subjects = ?')
        updateValues.push(JSON.stringify(body.subjects))
        console.log('✅ Subjects updated:', body.subjects)
      } catch (error) {
        if (error.message && (error.message.includes('no such column: subjects') || error.message.includes('has no column named subjects'))) {
    ...
    if (updateFields.length > 1) { // updated_at以外にも更新フィールドがある場合
      await db.prepare(`
        UPDATE teachers 
    ...
    if (body.subjects !== undefined) {
      ...
      // 既存の科目関連付けを削除
      const deleteResult = await db.prepare('DELETE FROM teacher_subjects WHERE teacher_id = ?').bind(teacherId).run()
      ...
      if (Array.isArray(body.subjects) && body.subjects.length > 0) {
        ...
        for (const subjectName of body.subjects) {
          ...
          console.log(`    📚 利用可能な科目（最初の10件）:`, allSubjects.results?.map(s => s.name))
          
          // 科目名から科目IDを取得（完全一致と部分一致の両方を試行）
          let subject = await db.prepare('SELECT id, name FROM subjects WHERE name = ?').bind(subjectName).first()
          console.log(`    📖 完全一致検索結果:`, subject)
          
          if (!subject) {
            // 部分一致でも試行
            subject = await db.prepare('SELECT id, name FROM subjects WHERE name LIKE ?').bind(`%${subjectName}%`).first()
          ...
          if (subject) {
            ...
            console.log(`    💾 関連付けを保存中: ${relationId} (teacher: ${teacherId}, subject: ${subject.id})`)
            console.log(`    🔍 バインド値確認:`)
            ...
            if (!relationId || !teacherId || !subject.id) {
              throw new Error(`Invalid bind values: relationId=${relationId}, teacherId=${teacherId}, subjectId=${subject.id}`)
            }
            ...
            try {
              const insertResult = await db.prepare(`
                INSERT INTO teacher_subjects (id, teacher_id, subject_id, created_at)
                VALUES (?, ?, ?, ?)
              `).bind(relationId, teacherId, subject.id, currentTime).run()
              console.log(`    ✅ 関連付け保存完了:`, insertResult.changes, '件', insertResult.meta)
            } catch (insertError) {
              console.log(`    ❌ 関連付け保存エラー:`, insertError)
              console.log(`    🔍 エラー詳細: relationId="${relationId}", teacherId="${teacherId}", subjectId="${subject.id}"`)
    ...
    const hasAssignmentRestrictionsColumn = updateTableInfo.results?.some(col => col.name === 'assignment_restrictions')
    ...
    // assignment_restrictionsフィールドをパース
    if (updatedTeacher.assignment_restrictions && typeof updatedTeacher.assignment_restrictions === 'string') {
      try {
...
// 教師削除（管理者権限必須）
schoolRoutes.delete('/teachers/:id', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validatePathParams(['id']),
  async (c) => {
  try {
    ...
    // 教師の存在確認
    const existing = await db.prepare('SELECT id, name FROM teachers WHERE id = ?').bind(teacherId).first()
    if (!existing) {
      return c.json({
        success: false,
        error: '指定された教師が見つかりません'
    ...
    const deleteSubjectsResult = await db.prepare('DELETE FROM teacher_subjects WHERE teacher_id = ?').bind(teacherId).run()
    console.log('📊 教科関連付け削除結果:', deleteSubjectsResult)
    
    // 教師を削除
    console.log('👨‍🏫 教師レコードを削除中...')
    const deleteTeacherResult = await db.prepare('DELETE FROM teachers WHERE id = ?').bind(teacherId).run()
...
schoolRoutes.put('/subjects/:id', 
  ...
  try {
    ...
    // 科目の存在確認
    const existing = await db.prepare('SELECT id FROM subjects WHERE id = ?').bind(subjectId).first()
    if (!existing) {
      return c.json({
        success: false,
    ...
    if (body.specialClassroom !== undefined) {
      ...
      const tableInfo = await db.prepare(`PRAGMA table_info(subjects)`).all()
      const hasSpecialClassroomColumn = tableInfo.results?.some(col => col.name === 'special_classroom')
      
      if (!hasSpecialClassroomColumn) {
        // Add specialClassroom column
        await db.prepare(`ALTER TABLE subjects ADD COLUMN special_classroom TEXT DEFAULT NULL`).run()
    ...
    const updatedSubject = await db.prepare('SELECT * FROM subjects WHERE id = ?').bind(subjectId).first()
    
    // targetGradesフィールドをパースして配列に変換し、weeklyLessonsフィールドをマッピング
    if (updatedSubject && updatedSubject.target_grades) {
      try {
...
// 科目順序の一括更新（管理者権限必須）
schoolRoutes.patch('/subjects/reorder', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validateRequestBody({
    subjects: { 
  ...
  try {
    ...
    const statements = body.subjects.map(subject => {
      return db.prepare('UPDATE subjects SET `order` = ?, updated_at = ? WHERE id = ?')
...
// 教師順序の一括更新（管理者権限必須）
schoolRoutes.patch('/teachers/reorder', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validateRequestBody({
    teachers: { 
  ...
  try {
    ...
    const statements = body.teachers.map(teacher => {
      return db.prepare('UPDATE teachers SET `order` = ?, updated_at = ? WHERE id = ?')
...
// 教室順序の一括更新（管理者権限必須）
schoolRoutes.patch('/classrooms/reorder', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validateRequestBody({
    classrooms: { 
  ...
  try {
    ...
    const statements = body.classrooms.map(classroom => {
      return db.prepare('UPDATE classrooms SET `order` = ?, updated_at = ? WHERE id = ?')
...
schoolRoutes.post('/subjects', 
  ...
  try {
    ...
    const newSubject = await db.prepare('SELECT * FROM subjects WHERE id = ?').bind(subjectId).first()

    // targetGradesフィールドをパースして配列に変換し、weeklyLessonsフィールドをマッピング
    if (newSubject && newSubject.target_grades) {
      try {
...
// 教科削除（管理者権限必須）
schoolRoutes.delete('/subjects/:id', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validatePathParams(['id']),
  async (c) => {
  try {
    ...
    // 教科の存在確認
    const existing = await db.prepare('SELECT id, name FROM subjects WHERE id = ?').bind(subjectId).first()
    if (!existing) {
      return c.json({
        success: false,
...
// 教室更新（管理者権限必須）
schoolRoutes.put('/classrooms/:id', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validatePathParams(['id']),
  validateRequestBody({
  ...
  try {
    ...
    // 教室の存在確認
    const existing = await db.prepare('SELECT id FROM classrooms WHERE id = ?').bind(classroomId).first()
    if (!existing) {
      return c.json({
        success: false,
    ...
    const updatedClassroom = await db.prepare('SELECT * FROM classrooms WHERE id = ?').bind(classroomId).first()
...
schoolRoutes.post('/classrooms', 
  ...
  try {
    ...
    const newClassroom = await db.prepare('SELECT * FROM classrooms WHERE id = ?').bind(classroomId).first()
...
// 教室削除（管理者権限必須）
schoolRoutes.delete('/classrooms/:id', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validatePathParams(['id']),
  async (c) => {
  try {
    ...
    // 教室の存在確認
    const existing = await db.prepare('SELECT id, name FROM classrooms WHERE id = ?').bind(classroomId).first()
    if (!existing) {
      return c.json({
        success: false,
        error: '教室が見つかりません'
    ...
    // 教室を削除
    const deleteResult = await db.prepare('DELETE FROM classrooms WHERE id = ?').bind(classroomId).run()
...
// 時間割詳細取得（認証必須）
schoolRoutes.get('/timetables/:id', 
  clerkAuthMiddleware, 
  readOnlyAuthMiddleware,
  validatePathParams(['id']),
  async (c) => {
  try {
    ...
    try {
      ...
      console.log('📋 データベース内の全時間割ID:', allTimetables.results?.map(t => ({ id: t.id, name: t.name })) || [])
    ...
    try {
      ...
      console.log('🏗️ テーブル構造:', tableInfo.results?.map(col => ({ name: col.name, type: col.type, notnull: col.notnull })))
    } catch (schemaError) {
    ...
    if (timetable.timetable_data && typeof timetable.timetable_data === 'string') {
      try {
        ...
        console.warn('📝 パース失敗データ (最初の100文字):', timetable.timetable_data?.substring(0, 100))
...
schoolRoutes.post('/timetables', 
  ...
  try {
    ...
    const newTimetable = await db.prepare(`
      SELECT 
...
// 時間割更新（管理者権限必須）
schoolRoutes.put('/timetables/:id', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validatePathParams(['id']),
  validateRequestBody({
  ...
  try {
    ...
    // 時間割の存在確認
    const existing = await db.prepare('SELECT id FROM timetables WHERE id = ?').bind(timetableId).first()
    if (!existing) {
      return c.json({
        success: false,
        error: '指定された時間割が見つかりません'
    ...
    const updatedTimetable = await db.prepare(`
      SELECT 
        id, name, description, academic_year, term,
        start_date, end_date, status, is_active,
        created_at, updated_at, version
...
// 時間割削除（管理者権限必須）
schoolRoutes.delete('/timetables/:id', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validatePathParams(['id']),
  async (c) => {
  try {
    ...
    // 時間割の存在確認
    const existing = await db.prepare('SELECT id, name FROM timetables WHERE id = ?').bind(timetableId).first()
    if (!existing) {
      return c.json({
        success: false,
        error: '指定された時間割が見つかりません'
...
// 教師-教科関連作成（管理者権限必須）
schoolRoutes.post('/teacher-subjects', 
  clerkAuthMiddleware, 
  adminAuthMiddleware,
  validateRequestBody({
...
schoolRoutes.get('/debug/timetables/:id', clerkAuthMiddleware, async (c) => {
  try {
    ...
    // 3. 指定IDのレコード詳細確認
    const targetRecord = await db.prepare("SELECT * FROM timetables WHERE id = ?").bind(timetableId).first()
    ...
    // 5. JSON パーステスト
    let parseTest = null
    if (targetRecord && targetRecord.timetable && typeof targetRecord.timetable === 'string') {
      try {
