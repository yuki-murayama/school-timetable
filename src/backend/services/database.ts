import { GenerationSession, GenerationStep, Timetable, TimetableData } from '../types'
...
export class DatabaseService {
  ...
  async createMasterTables(): Promise<void> {
    ...
    // すべての既存テーブルを削除
    const tables = [
      'school_settings', 'teachers', 'subjects', 'classrooms',
      'teacher_subjects', 'users', 'schools', 'classes',
      'classroom_subjects', 'schedules', 'timetables'
    ]
    ...
    try {
      ...
      if (!columns.includes('weekly_hours')) {
        ...
        await this.db.prepare(`ALTER TABLE subjects ADD COLUMN weekly_hours TEXT DEFAULT '{}'`).run()
      ...
      if (!columns.includes('requires_special_classroom')) {
        ...
        await this.db.prepare(`ALTER TABLE subjects ADD COLUMN requires_special_classroom INTEGER DEFAULT 0`).run()
      ...
      if (!columns.includes('classroom_type')) {
        ...
        await this.db.prepare(`ALTER TABLE subjects ADD COLUMN classroom_type TEXT DEFAULT 'normal'`).run()
    ...
    try {
      ...
      const teacherColumns = (teacherTableInfo.results || []).map((col: any) => col.name)
      ...
      for (const column of requiredColumns) {
        if (!teacherColumns.includes(column.name)) {
          ...
          await this.db.prepare(`ALTER TABLE teachers ADD COLUMN ${column.name} ${column.type} DEFAULT ${column.defaultValue}`).run()
    ...
    try {
      ...
      const classroomColumns = (classroomTableInfo.results || []).map((col: any) => col.name)
      ...
      console.log('📊 マイグレーション後のclassroomsテーブル構造:', updatedClassroomTableInfo.results)
  ...
  async insertDefaultData(): Promise<void> {
    ...
    for (const teacher of teachers) {
      await this.db.prepare(`
        ...
        VALUES (?, ?, ?)
      `).bind(teacher.id, teacher.name, teacher.subjects).run()
    }
    ...
    for (const subject of subjects) {
      await this.db.prepare(`
        ...
        VALUES (?, ?)
      `).bind(subject.id, subject.name).run()
    }
    ...
    try {
      ...
      // 新しいスキーマでのデータ更新
      const updatedColumns = (await this.db.prepare(`PRAGMA table_info(subjects)`).all()).results.map((col: any) => col.name)
    ...
    for (const classroom of classrooms) {
      await this.db.prepare(`
        ...
        VALUES (?, ?, ?)
      `).bind(classroom.id, classroom.name, classroom.capacity).run()
    }
  ...
  async createSessionTables(): Promise<void> {
    // generation_sessions テーブル作成
    await this.db.prepare(`
      CREATE TABLE IF NOT EXISTS generation_sessions (
        id TEXT PRIMARY KEY,
        status TEXT NOT NULL DEFAULT 'active',
        current_day TEXT NOT NULL,
        current_class_index INTEGER NOT NULL DEFAULT 0,
  ...
  async createSession(sessionId: string, totalSteps: number): Promise<void> {
    await this.db.prepare(`
      INSERT INTO generation_sessions (id, status, current_day, current_class_index, completed_steps, total_steps, created_at, updated_at)
      VALUES (?, 'active', 'monday', 0, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  ...
  async getSession(sessionId: string): Promise<any> {
    ...
    console.log(`📋 セッションデータ取得: ${sessionId}`, session)
    return session
  }

  async updateSession(sessionId: string, updates: Partial<GenerationSession>): Promise<void> {
    console.log(`🔄 セッション更新開始: ${sessionId}`, updates)
  ...
  async saveStep(step: Omit<GenerationStep, 'created_at'>): Promise<void> {
    await this.db.prepare(`
  ...
  async getStepsForDay(sessionId: string, day: string): Promise<any[]> {
    const result = await this.db.prepare(`
      SELECT day, class_id, grade, class_number, step_data 
      FROM generation_steps 
      WHERE session_id = ? AND day = ?
      ORDER BY grade, class_number
    `).bind(sessionId, day).all()

    return result.results || []
  }

  async getAllSteps(sessionId: string): Promise<any[]> {
    const result = await this.db.prepare(`
  ...
  async saveTimetable(timetable: any): Promise<{ id: string }> {
  ...
  async collectTimetableData(): Promise<TimetableData> {
    ...
    try {
      // まず新しいスキーマでトライ
      subjectsResult = await this.db.prepare('SELECT id, name, weeklyHours, targetGrades, created_at, updated_at FROM subjects ORDER BY name').all()
      console.log('✅ 新しいスキーマでsubjects取得成功')
    } catch (error) {
      console.log('📊 新しいスキーマ失敗、古いスキーマでリトライ:', error)
      // 古いスキーマでフォールバック
      subjectsResult = await this.db.prepare('SELECT id, name, created_at, updated_at FROM subjects ORDER BY name').all()
      console.log('✅ 古いスキーマでsubjects取得成功')
    }
    
    // 他のテーブルは並列で取得
    const [teachersResult, classroomsResult, schoolSettingsResult, conditionsResult] = await Promise.all([
    ...
    for (let grade = 1; grade <= 3; grade++) {
      const classCount = schoolSettings[`grade${grade}Classes` as keyof typeof schoolSettings] as number
